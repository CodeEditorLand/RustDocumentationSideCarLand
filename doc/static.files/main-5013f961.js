...
(function () {
		...
		setup: () => {
			...
			function loadSearch() {
				if (!searchLoaded) {
					...
					loadScript(
						resourcePath("search.index/root", ".js"),
						sendSearchForm,
					);
				}
			}
			...
			window.onpageshow = () => {
				const inputElement = window.searchState.inputElement();
				const qSearch =
					window.searchState.getQueryStringParams().search;
	...
	window.register_implementors = (imp) => {
		...
		const syntheticImplementors = implementorsElems(
		...
		if (syntheticImplementors[0]) {
			onEachLazy(
				syntheticImplementors[0].getElementsByClassName("impl"),
				(el) => {
		...
		let currentNbImpls =
			implementors[0].getElementsByClassName("impl").length;
		...
		for (const impList of imp[nonnull(window.currentCrate)]) {
			...
			onEachLazy(template.content.querySelectorAll("[id]"), (el) => {
				...
				if (i !== 0) {
					...
					el.id = `${el.id}-${i}`;
					onEachLazy(
						template.content.querySelectorAll("a[href]"),
						(link) => {
...
<a href="${drloChannel}/rustdoc/">the rustdoc book</a>.`;
	...
	window.hidePopoverMenus = () => {
		onEachLazy(
			document.querySelectorAll(".settings-menu .popover"),
			(elem) => {
	...
	function copyCode(codeElem) {
		...
		for (const node of codeElem.childNodes) {
			if (
				...
				node.hasAttribute("data-nosnippet")
			) {
				continue;
			}
