...
const initSearch = async function (Stringdex, RoaringBitmap, hooks) {
		...
		while (parserState.pos < parserState.length) {
			...
			if (!foundStopChar) {
				...
				if (isLastElemGeneric(query.elems, parserState)) {
					...
					throw [
						"Cannot have more than one element if you use quotes",
					];
				}
				if (endChar !== "") {
					throw [
	...
	function getNextElem(query, parserState, elems, isInGenerics) {
		...
		const handleRefOrPtr = (chr, name) => {
			...
			if (parserState.userQuery.slice(pos, pos + 3) === "mut") {
				generics.push(
					makePrimitiveElement("mut", { typeFilter: "keyword" }),
				);
				parserState.pos += 3;
				c = parserState.userQuery[parserState.pos];
			} else if (
		...
		if ("[(".indexOf(parserState.userQuery[parserState.pos]) !== -1) {
			...
			if (
				...
				elems.push(
					makePrimitiveElement(name, { bindingName, generics }),
				);
			}
	...
	class DocSearch {
		...
		async getTypeNameIdsAsync(nn) {
			...
			const typeNameIdOfFn = await first(
				fn,
				itemTypes.trait,
				"core::ops",
			);
			...
			const typeNameIdOfArray = await first(
				array,
				itemTypes.primitive,
				"",
			);
			const typeNameIdOfSlice = await first(
				slice,
				itemTypes.primitive,
				"",
			);
			...
			const typeNameIdOfTuple = await first(
				tuple,
				itemTypes.primitive,
				"",
			);
			...
			const typeNameIdOfPointer = await first(
				pointer,
				itemTypes.primitive,
				"",
			);
			const typeNameIdOfHof = await first(hof, itemTypes.primitive, "");
			const typeNameIdOfNever = await first(
				never,
				itemTypes.primitive,
				"",
			);
		...
		async getPathData(id) {
			...
			return {
				...
				modulePath: raw[1],
				exactModulePath:
					raw[2] === 0 || raw[2] === undefined ? raw[1] : raw[2],
			};
		}
		async getFunctionData(id) {
			...
			const raw = JSON.parse(encoded);
			const parser = new VlqHexDecoder(
				raw[0],
				async (functionSearchType) => {
					...
					if (functionSearchType.length > 1) {
						if (
							typeof functionSearchType[OUTPUT_DATA] === "number"
						) {
							output_ = Promise.all([
								this.buildItemSearchType(
		...
		async getCrateNameList() {
			...
			for (let i = 0; i < l; ++i) {
				...
				names.push(
					name === undefined ? "" : this.utf8decoder.decode(name),
				);
			}
		...
		async buildItemSearchTypeAll(types) {
			return types && types.length > 0
				? await Promise.all(
						types.map((type) => this.buildItemSearchType(type)),
					)
		...
		async buildItemSearchType(type) {
			...
			if (typeof type === "number") {
				...
				generics = await this.buildItemSearchTypeAll(
					type[GENERICS_DATA],
				);
		...
		async execQuery(parsedQuery, filterCrates, currentCrate) {
			...
			const buildHrefAndPath = (item) => {
				...
				if (type === "mod") {
					...
					if (parentType === "primitive") {
						...
						const enumName = item.modulePath.substr(
							enumNameIdx + 2,
						);
					...
					if (
						item.entry &&
						item.entry.associatedItemDisambiguator !== null
					) {
						anchor =
							item.entry.associatedItemDisambiguator +
							"/" +
							anchor;
					}
					href =
					...
					if (
						...
						const [ob, sb] =
							...
							fnType.id === typeNameIds.typeNameIdOfSlice
								...
								: ["(", ")"];
						pushText(
							{ name: ob, highlighted: fnType.highlighted },
							result,
						);
						...
						pushText(
							{ name: sb, highlighted: fnType.highlighted },
							result,
						);
						return true;
					} else if (
						fnType.id === typeNameIds.typeNameIdOfReference
					) {
						pushText(
							{ name: "&", highlighted: fnType.highlighted },
							result,
						);
			...
			const transformResults = (results, typeInfo, duplicates) => {
				...
				for (const result of results) {
					...
					if (item.id !== -1) {
						...
						if (
							obj.item.ty === itemTypes.import &&
							duplicates.has(res[2])
						) {
							continue;
						}
						...
						if (obj.traitPath) {
							...
							const toRemoveList = traitImplIdxMap.get(
								obj.fullPath,
							);
							if (toRemoveList) {
						...
						const highlightedFnType = Object.assign({}, fnType, {
							...
							bindings: new Map(
								[...fnType.bindings.entries()].map(([k, v]) => {
									return [
										k,
										highlightedGenerics.splice(0, v.length),
									];
								}),
							),
						});
					...
					if (
						...
						if (fnTypesRemaining) {
							...
							highlighted[0] = Object.assign(
								...
								{
									generics:
										unifiedGenerics || fnType.generics,
								},
							);
							return highlighted;
						}
			...
			const unifyFunctionTypeIsMatchCandidate = (
				...
				if (
					...
					const queryElemPathLength =
						queryElem.pathWithoutLast.length;
			...
			function containsTypeFromQuery(
				elems,
				list,
				where_clause,
				typeNameIds,
				...
				pathiter: for (let i = length - clength; i >= 0; i -= 1) {
					...
					for (let x = 0; x < clength; ++x) {
						...
						if (parsedQuery.literalSearch && p !== c) {
							...
							Math.floor((p.length - c.length) / 3) <=
								maxPathEditDistance &&
							p.indexOf(c) !== -1
						) {
			...
			const innerRunNameQuery = async function* (currentCrate) {
				...
				const handleAlias = async (name, alias, dist, index) => {
					...
					const path_dist =
						name.includes(" ") || parsedQuery.elems.length === 0
							? 0
							: checkRowPath(
									parsedQuery.elems[0].pathWithoutLast,
									item,
								);
					if (path_dist === null) {
						return null;
				...
				const handleNameSearch = async (id) => {
					...
					if (parsedQuery.literalSearch) {
						...
						return {
							...
							path_dist: pathDist,
							index: row.normalizedName.indexOf(
								elem.normalizedPathLast,
							),
							elems: [],
							returned: [],
				...
				const levSearchResults = index.searchLev(
					elem.normalizedPathLast,
				);
				const levResults = [];
			...
			const innerRunTypeQuery = async function* (
				...
				const nested_everything_postings_list = [
					everything_postings_list,
				];
				...
				const unpackPostingsList = async (elem, polarity) => {
					...
					if (types.length === 0) {
						...
						if (
							...
							const levResults = index.searchLev(
								elem.normalizedPathLast,
							);
						...
						if (areGenericsAllowed) {
							...
							return [
								{
									invertedIndex:
										await this.getGenericInvertedIndex(
											genericId,
										),
									queryElem: {
										name: elem.name,
					...
					types.sort(
						(
							[_i, name1, _t, pathData1],
							[_i2, name2, _t2, pathData2],
						) => {
				...
				plan: for (const queryStep of queryPlan) {
					for (const { bitmap, inputs, output } of queryStep) {
						for (const id of bitmap.entries()) {
							...
							resultPromises.push(
								this.getFunctionData(id).then(
									async (fnData) => {
										if (
											!fnData ||
											!fnData.functionSignature
										) {
											return null;
										}
										checkCounter += 1;
	...
	async function addTab(
		...
		const addNextResultToOutput = async (obj) => {
			...
			if (obj.item.deprecated) {
				...
				deprecatedCountElem.innerText = `${deprecatedCount} deprecated item${plural} hidden by setting`;
			}
			link.href = obj.href;
			...
			if (obj.displayTypeSignature) {
				...
				if (mappedNames.size > 0 || whereClause.size > 0) {
					...
					for (const [qname, name] of mappedNames) {
						...
						line.className = "where";
						line.appendChild(
							document.createTextNode(`    ${qname} matches `),
						);
		...
		searchState.outputElement().addEventListener("keydown", (e) => {
			...
			if (e.which === 38) {
				...
				if (active) {
					...
					if (
						previous &&
						previous.className !== DEPRECATED_COUNT_SELECTOR
					) {
						previous.focus();
					} else {
						searchState.focus();
